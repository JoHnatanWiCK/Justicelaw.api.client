document.addEventListener('DOMContentLoaded', function () {
    const BASE_URL = 'https://apijusticelaw-production.up.railway.app';
    ...
    async function toggleLike(notificationId, corazon) {
        try {
            ...
            const response = await fetch(`${NOTIFICATIONS_URL}/${notificationId}/like`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Error al intentar dar me gusta');
            }
    ...
    async function fetchNotificationsAction(action, notificationId = null) {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.message || `Error al realizar la acción: ${action}`);
            }
