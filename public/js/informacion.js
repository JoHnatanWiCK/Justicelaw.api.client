        ...
        informations.forEach((info) => {
            ...
            readMoreButton.addEventListener("click", async (event) => {
                ...
                try {
                    ...
                    const imageHTML = `
                        <img src="${data.cover_photo ? data.cover_photo : "../../img/placeholder.png"}" 
                             alt="${data.name || "Imagen no disponible"}" class="informacion-imagen" 
                             onerror="this.onerror=null;this.src='../../img/placeholder.png';">
                    `;
...
async function registrarVista(informacionId) {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 'No se pudo registrar la vista.');
        }
